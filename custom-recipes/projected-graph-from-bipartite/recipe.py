...
logging.info("Bipartite Graph - Dataset loaded...")

# Delete nulls
df = df[(df[params[Constants.GRAPH_OF]].notnull()) & (df[params[Constants.LINKED_BY]].notnull())]
logging.info("Bipartite Graph - Removed null values...")

# Dedup
dd = df.groupby(columns).size().reset_index().rename(columns={0: 'w'})
logging.info("Bipartite Graph - Created deduplicated dataset...")
...
logging.info("Bipartite Graph - Created bipartite graph...")
...
logging.info("Bipartite Graph - Created projected graph...")

# Recipe outputs
logging.info("Bipartite Graph - Writing output dataset...")
